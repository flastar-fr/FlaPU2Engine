...
public:
    ...
    void execute(Engine& engine) const override {
        ...
        if (isAddressesRegValuesHighLow()) {
            const uint8_t high_address_value = registers[operands[0].value].getValue();
            const uint8_t low_address_value = registers[operands[1].value].getValue();
            const uint8_t value_to_store = registers[operands[2].value].getValue();

            const uint16_t address_to_read_from = (high_address_value << AMOUNT_BITS_PER_CELL) + low_address_value;

            memory[address_to_read_from] = value_to_store;
        }
    }
    ...
    [[nodiscard]] bool isAddressesRegValuesHighLow() const {
        ...
        const bool first_is_register_value_high = operands[0].value_type == ValueType::REGISTER_VALUE;
        const bool second_is_register_value_low = operands[1].value_type == ValueType::REGISTER_VALUE;
        const bool third_is_register_value = operands[2].value_type == ValueType::REGISTER_VALUE;

        return first_is_register_value_high && second_is_register_value_low && third_is_register_value;
    }
};
